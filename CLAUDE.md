...
|-------------|--------------------------------------|-----------------------------------------------|
| Background  | src/entrypoints/background.ts        | Service worker: CSP, injection, menus, proxy  |
...
| CSP Bypass  | src/entrypoints/csp-bypass.content/  | Content script: fetch proxy for CSP           |
...
- CSP bypass fetch proxy routes requests through background for specific domains
- Environment variables injected as `window.__ST_ENV__` for source scripts
